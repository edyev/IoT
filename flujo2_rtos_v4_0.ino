...
static void TaskFlow(void* pvParameters)
{
  ...
  while(1){
      /*Reads pulse duration from ISR and stores in data
      Both ISR can write to the queue*/      
      xQueuePeek(xQueue, &(data), (TickType_t )0);
      /*Condition that marks end of an event, value in uS*/
      if(data > 65500){
         /*Threshold for number of pulses that mark a valid event*/
         if(TCNT1 > 10){
          volume = pulses_2_lt(TCNT1);
          /*Condition to disregard null volumes and validate queue space*/
          if(uxQueueMessagesWaiting(xEventQueue) < 12 && volume > 0){
            /*Sends volume through the queue*/
...
void timerCounterSetup() {
  ...
  /*Enables 8-bit Timer2 Overflow interruption*/
  bitSet(TIMSK2, 0);
...
void trigger_int() {
  ...
  /*Variable to verify if task switch is needed*/
  ...
  if (EICRA == 0x0C){
    ...
    detachInterrupt(digitalPinToInterrupt(interruptPin));
    TCCR2B =0;
    /*Manipulation to implement 16-bit counter using 8-bit Timer2*/
    duration = (time_counter << 8) | TCNT2;
    /*Send duration through queue xQueue*/
    xQueueOverwriteFromISR(xQueue,(void*) &duration, pxHigherPriorityTaskWoken);
    time_counter = 0;
    TCNT2 = 0;
    /*Configure Timer2, lsb = 1uS*/
    bitSet(TCCR2B, CS21);
    /*Configure same interrupt but for falling edge*/
    attachInterrupt(digitalPinToInterrupt(interruptPin), trigger_int, FALLING);
...
void Send_Sensors() {
  ...
  const char *ending = "\n";
  /*Variable to receive xEventQueue data*/
