...
{   
    ...
    xTaskCreate(TaskDownlink, "TaskDownlink", 500, NULL, 4, &DownlinkHandle);
     /*Create keeper task*/
    xTaskCreate(TaskKeeper, "TaskKeeper", 100, NULL, 3, &KeeperHandle);
    /*Create task that sends the data through Sigfox, Required Stack: 134*/
...
static void TaskKeeper(void* pvParameters){
  while(1){
    if(EEPROM.read(10) > 5)
...
void Downlink(){
  ...
  uint8_t timeout = 0;
  while(1){
  if(Serial.available() || timeout > 12000){
    _aux = Serial.read();
...
uint8_t verify_connection(){
  /*Function to verify connection with Sigfox via downlink
    Once the downlink is fetched or timeouts this checks for
    a special key inside the downlink bytes*/
  return 1;
}
