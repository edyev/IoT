...
void setup()
{   
    /*Enable Serial3 communication*/
   
    //while(!Serial3);
    Serial3.begin(9600);
    Serial.begin(9600);
    /*Create mutext to prevent two tasks accessing Serial3 port*/
    do{
    ...
    while(1){
        ...
        if (!(event_active)  && (event_index < 12))
        {
            ...
            if(TCNT5>10){
                  ...
                  TCNT5 = 0;
                  //Serial3.print("event_index: ");
                  //Serial3.println(event_index);
                  event_index += 1; 
                  /*Event finished*/
                  terminate_flag = 1;
...
void Downlink(){
  ...
  if(Serial3.available() || timeout > 12000){
    Serial.print(_aux);
    _aux = Serial3.read();
    
    if((_aux == 'R') || (timeout > 12000)) {
      
      uint8_t n = Serial3.readBytesUntil('\n', rx_data,50);
      if (n < 8)
        dl_error = 1;
      else
        dl_error = 0;
...
bool key_parse(char* downlink){
  ...
  for(uint8_t i = 0; i < sizeof(rx_data); ++i){
    ...
    rx_data[i] = 'x';
     ...
     // Serial3.println(atoi(_downlink[1]));
